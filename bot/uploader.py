...
class TelegramUploader:
    ...
    async def upload_and_cleanup(
        ...
        try:
            ...
            err_text = str(e)
            if 'Request Entity Too Large' in err_text or 'request entity too large' in err_text:
                # IMPORTANT: In this build we never post direct links as fallback.
                # If Telegram rejects the upload (413), we simply skip the item.
                try:
                    size_bytes = os.path.getsize(local_path) if local_path and os.path.exists(local_path) else 0
                except Exception:
                    size_bytes = 0
                logger.warning(
                    "Telegram 413 (Request Entity Too Large) for url=%s size_bytes=%s. Skipping (no link fallback).",
