...
class VIPBot:
    ...
    def __init__(self):
        ...
        # Supervisor: keep periodic tasks alive all day (auto previews + VIP feed)
    ...
    async def _admin_only(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """True if the user is allowed to use admin UI/commands.

        Delegates to admin.admin_only() so we keep exactly the same admin policy everywhere.
        """
        try:
            return bool(await admin_only(update, context))
        except Exception as e:
            logger.warning('admin_only check failed: %s', e)
            return False
    ...
    def _coomer_media_key_str(self, creator: dict, media_item: MediaItem) -> str:
        """Stable string key for global de-duplication across VIP + FREE."""
        try:
            service = str(creator.get("service") or "")
            cid = str(creator.get("id") or "")
            post_id = str(getattr(media_item, "post_id", "") or "")
            url = str(getattr(media_item, "url", "") or "")
            fname = str(getattr(media_item, "filename", "") or "")
            raw = f"coomer|{service}|{cid}|{post_id}|{fname}|{url}".encode("utf-8", "ignore")
            return hashlib.sha1(raw).hexdigest()
        except Exception:
            try:
                return hashlib.sha1(repr(media_item).encode("utf-8", "ignore")).hexdigest()
            except Exception:
                return str(self._coomer_item_key(creator, media_item))
    ...
    async def _send_random_previews_from_coomer(self, bot, model: str, qty: int, dest_chat_ids: list[int]) -> Tuple[int, int]:
        ...
        Slot engine (A):
        - 1 creator per slot, but can rotate if a creator yields 0 items for the cycle.
        - Per-destination rotation memory prevents getting stuck on a single model.
        - Per-destination per-creator progress keeps pagination consistent when returning to a model.
        - Global de-dup prevents repeats across VIP + FREE for automatic modules.
        """
        sent_count = 0
        error_count = 0
        ...
        vip_chat_id = config.get_value("VIP_CHANNEL_ID", getattr(config, "VIP_CHANNEL_ID", 0)) or 0

        def _creator_key(c: dict) -> str:
            return f"{str(c.get('service') or '').strip().lower()}:{str(c.get('id') or c.get('username') or '').strip()}"

        def _creator_score(c: dict) -> float:
            score = 0.0
            for k in ("favorited", "favorites", "followers", "subscribers", "likes", "posts", "post_count"):
                v = c.get(k)
                try:
        ...
        def _weighted_choice(pool: list[dict], exclude_key: str | None = None) -> dict | None:
            if not pool:
                return None
            candidates = pool
            if exclude_key:
                tmp = [c for c in pool if _creator_key(c) != exclude_key]
                if tmp:
                    candidates = tmp
            weights = [_creator_score(c) for c in candidates]
            try:
                return random.choices(candidates, weights=weights, k=1)[0]
            except Exception:
                return random.choice(candidates)

        model_name = (model or "all").strip()
        model_lc = model_name.lower()

        async with MediaFetcher() as fetcher:
            try:
                top_creators = await self._coomer_get_top_creators(fetcher, top_n=200)
            except Exception:
                top_creators = []
            if not top_creators:
                return 0, 1

            svc_filtered = [c for c in top_creators if str(c.get("service") or "").lower() in ("onlyfans", "fansly")]
            if svc_filtered:
                top_creators = svc_filtered

            head_k = 25
            head = top_creators[: min(head_k, len(top_creators))] if top_creators else []

            resolved_specific: dict | None = None
            if model_lc not in ("all", "random", "popular", "*"):
                for c in top_creators[:1000]:
                    n = str(c.get("name") or c.get("username") or "").strip()
                    if n and n.lower() == model_lc:
                        resolved_specific = c
                        break
                if resolved_specific is None:
                    try:
                        all_creators = await fetcher.get_creators()
            ...
            for dest_id in dest_chat_ids:
                dest_id = int(dest_id)
                dest_kind = "vip" if vip_chat_id and dest_id == int(vip_chat_id) else "free"

                sent_for_dest = 0
                creators_attempted = 0

                if resolved_specific:
                    creator = resolved_specific
                    last_key = None
                else:
                    try:
                        last_key = self.preview_db.get_coomer_state(dest_kind, dest_id)
                    except Exception:
                        last_key = None
                    pool = head if head else top_creators[: min(50, len(top_creators))]
                    creator = _weighted_choice(pool, exclude_key=last_key)

                while sent_for_dest < int(qty) and creators_attempted < 3 and creator:
                    creators_attempted += 1
                    creator_key = _creator_key(creator)
                    creator_name = str(creator.get("name") or creator.get("username") or creator.get("id") or "modelo")
                    service = str(creator.get("service") or "").strip().lower()
                    creator_id = str(creator.get("id") or creator.get("username") or "").strip()

                    try:
                        offset = int(self.preview_db.get_coomer_progress(dest_kind, dest_id, service, creator_id) or 0)
                    except Exception:
                        offset = 0

                    pages_tried = 0
                    last_offset = int(offset or 0)

                    while sent_for_dest < int(qty) and pages_tried < 10:
                        pages_tried += 1
                        try:
                            media_items: list[MediaItem] = await fetcher.fetch_posts_paged(creator, offset=int(last_offset), smart_sort=True)
                        except Exception as e:
                            logger.warning("Coomer autopreviews: fetch_posts_paged failed (%s): %s", creator_name, e)
                            media_items = []
                        ...
                        for item in media_items:
                            if sent_for_dest >= int(qty):
                                break
                            try:
                                gkey = self._coomer_global_key(creator, item)
                            except Exception:
                                gkey = None

                            if gkey:
                                try:
                                    if self.preview_db.global_has_sent(gkey):
                                        continue
                                except Exception:
                                    pass

                            # Coomer fetch returns URL metadata only; we must download before uploading.
                            # Without this, uploader will see local_path=None and nothing is sent.
                            try:
                                try:
                                    ok_dl = await fetcher.download_media(item)
                                except TooLargeMedia as tl:
                                    # Skip oversized files entirely (no link fallback).
                                    try:
                                        logger.info(
                                            "Auto previews: skipping oversized media (%.2f MB > %.2f MB) url=%s",
                                ...
                                logger.warning("Coomer autopreviews: download failed (%s): %s", creator_name, e)
                                continue

                            caption = f"🔥 Preview ({creator_name})"
                            try:
                                ok_up = await uploader.upload_and_cleanup(
                                    item,
                                    channel_id=int(dest_id),
                                    caption=caption,
                                    model_name=creator_name,
                                )
                            except Exception as e:
                                logger.warning("Coomer autopreviews: upload failed (%s -> %s): %s", creator_name, dest_id, e)
                                ok_up = False
                            ...
                            if ok_up:
                                ...
                                try:
                                    if gkey:
                                        self.preview_db.global_mark_sent(gkey, source="coomer")
                                except Exception:
                                    pass
                    ...
                    try:
                        if service and creator_id:
                            self.preview_db.set_coomer_progress(dest_kind, dest_id, service, creator_id, int(last_offset))
                    except Exception:
                        pass

                    # Update state for rotation (even if we sent 0, so we move on next cycle)
                    try:
                        self.preview_db.set_coomer_state(dest_kind, dest_id, creator_key)
                    except Exception:
                        pass

                    if sent_for_dest > 0 or resolved_specific:
                        break

                    # rotate to another creator when this one yields 0
                    pool = head if head else top_creators[: min(50, len(top_creators))]
                    creator = _weighted_choice(pool, exclude_key=creator_key)
    ...
    async def _send_random_previews(self, bot, model: str, qty: int, dest_chat_ids: list[int]) -> Tuple[int, int]:
        ...
        for dest in dest_chat_ids:
            ...
            for it in items:
                try:
                    # Global de-dup for automatic modules: prevent the same VIP message
                    # from being sent to multiple FREE destinations (and re-sent later).
                    gkey = f"vip|{int(it['vip_chat_id'])}|{int(it['vip_message_id'])}"
                    try:
                        if self.preview_db.global_has_sent(gkey):
                            continue
                    except Exception:
                    ...
                    try:
                        self.preview_db.global_mark_sent(gkey, source="vip_forward")
                    except Exception:
                        pass
                    sent_count += 1
                except Exception as e:
    ...
    async def on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "admin:auto" or data == "admin:auto:toggle":
            ...
            keyboard = InlineKeyboardMarkup([
                ...
                [
                    InlineKeyboardButton("⚙️ Config FREE", callback_data="admin:auto:ap"),
                    InlineKeyboardButton("⚙️ Config VIP", callback_data="admin:auto:vf"),
                ],
                [InlineKeyboardButton("⬅️ Voltar", callback_data="admin:menu")],
            ])
        ...
        # Admin: configure AutoPreviews (FREE) / VIP Feed (GUI)
        if data.startswith("admin:auto:ap"):
            if not config.is_authorized(user_id):
                return
            ap_cfg = self.preview_db.get_auto_config() or {}

            parts = data.split(":")
            if len(parts) >= 6 and parts[0] == "admin" and parts[1] == "auto" and parts[2] == "ap" and parts[3] == "set":
                field = parts[4]
                value = parts[5]
                try:
                    enabled = int(ap_cfg.get("enabled", 0) or 0)
                    interval_minutes = int(ap_cfg.get("interval_minutes", 1440) or 1440)
                    qty = int(ap_cfg.get("qty", 5) or 5)
                    model = str(ap_cfg.get("model", "all") or "all")
                    send_to_free = int(ap_cfg.get("send_to_free", 1) or 0)
                    send_to_private = int(ap_cfg.get("send_to_private", 0) or 0)
                    free_mode = str(ap_cfg.get("free_mode", "all") or "all")
                    ...
                    if field == "interval":
                        ...
                        model = str(value)
                    elif field == "send_to_free":
                        send_to_free = 1 if str(value) == "1" else 0
                    elif field == "send_to_private":
                        send_to_private = 1 if str(value) == "1" else 0
                    ...
                    ap_cfg = self.preview_db.get_auto_config() or {}
                    if int(ap_cfg.get("enabled", 0) or 0) == 1:
                        self._start_auto_previews_task()
                except Exception as e:
                    logger.warning("Admin auto previews GUI update failed: %s", e)
            ...
            text_msg = (
                "⚙️ Config FREE AutoPreviews\n\n"
                f"enabled: {ap_cfg.get('enabled')}\n"
                f"interval_minutes: {interval_minutes}\n"
                f"qty: {qty}\n"
                f"model: {model}\n"
                f"send_to_free: {send_to_free}\n"
                f"send_to_private: {send_to_private}\n"
            )
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("1m", callback_data="admin:auto:ap:set:interval:1"),
                    InlineKeyboardButton("2m", callback_data="admin:auto:ap:set:interval:2"),
                    InlineKeyboardButton("5m", callback_data="admin:auto:ap:set:interval:5"),
                    InlineKeyboardButton("10m", callback_data="admin:auto:ap:set:interval:10"),
                ],
                [
                    InlineKeyboardButton("30m", callback_data="admin:auto:ap:set:interval:30"),
                    InlineKeyboardButton("60m", callback_data="admin:auto:ap:set:interval:60"),
                    InlineKeyboardButton("240m", callback_data="admin:auto:ap:set:interval:240"),
                    InlineKeyboardButton("1440m", callback_data="admin:auto:ap:set:interval:1440"),
                ],
                [
                    InlineKeyboardButton("qty 5", callback_data="admin:auto:ap:set:qty:5"),
                    InlineKeyboardButton("qty 10", callback_data="admin:auto:ap:set:qty:10"),
                    InlineKeyboardButton("qty 15", callback_data="admin:auto:ap:set:qty:15"),
                    InlineKeyboardButton("qty 20", callback_data="admin:auto:ap:set:qty:20"),
                ],
                [
                    InlineKeyboardButton("model all", callback_data="admin:auto:ap:set:model:all"),
                    InlineKeyboardButton("model random", callback_data="admin:auto:ap:set:model:random"),
                ],
                [
                    InlineKeyboardButton(f"send_to_free {'✅' if send_to_free else '❌'}", callback_data=f"admin:auto:ap:set:send_to_free:{0 if send_to_free else 1}"),
                    InlineKeyboardButton(f"send_to_private {'✅' if send_to_private else '❌'}", callback_data=f"admin:auto:ap:set:send_to_private:{0 if send_to_private else 1}"),
                ],
                [
                    InlineKeyboardButton("⬅️ Voltar", callback_data="admin:auto"),
                ],
            ])
            try:
                await query.edit_message_text(text_msg, parse_mode=None, reply_markup=keyboard)
        ...
        if data.startswith("admin:auto:vf"):
            if not config.is_authorized(user_id):
                return
            vf_cfg = self.preview_db.get_vip_feed_config() or {}

            parts = data.split(":")
            if len(parts) >= 6 and parts[0] == "admin" and parts[1] == "auto" and parts[2] == "vf" and parts[3] == "set":
                field = parts[4]
                value = parts[5]
                try:
                    enabled = int(vf_cfg.get("enabled", 0) or 0)
                    interval_minutes = int(vf_cfg.get("interval_minutes", 60) or 60)
                    page_items = int(vf_cfg.get("page_items", 10) or 10)
                    top_n = int(vf_cfg.get("top_n", 100) or 100)
                    mode = str(vf_cfg.get("mode", "page") or "page")
                    head_k = int(vf_cfg.get("head_k", 25) or 25)

                    if field == "interval":
                        interval_minutes = int(value)
                    elif field == "qty":
                        page_items = int(value)
                    elif field == "top_n":
                        top_n = int(value)
                    elif field == "mode":
                        mode = str(value)
                    ...
                    vf_cfg = self.preview_db.get_vip_feed_config() or {}
                    if int(vf_cfg.get("enabled", 0) or 0) == 1:
                        self._start_vip_feed_task()
                except Exception as e:
                    logger.warning("Admin VIP feed GUI update failed: %s", e)
            ...
            text_msg = (
                "⚙️ Config VIP Feed\n\n"
                f"enabled: {vf_cfg.get('enabled')}\n"
                f"interval_minutes: {interval_minutes}\n"
                f"qty(page_items): {page_items}\n"
                f"top_n: {top_n}\n"
                f"mode: {mode}\n"
                f"head_k: {head_k}\n"
            )
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("1m", callback_data="admin:auto:vf:set:interval:1"),
                    InlineKeyboardButton("2m", callback_data="admin:auto:vf:set:interval:2"),
                    InlineKeyboardButton("5m", callback_data="admin:auto:vf:set:interval:5"),
                    InlineKeyboardButton("10m", callback_data="admin:auto:vf:set:interval:10"),
                ],
                [
                    InlineKeyboardButton("30m", callback_data="admin:auto:vf:set:interval:30"),
                    InlineKeyboardButton("60m", callback_data="admin:auto:vf:set:interval:60"),
                    InlineKeyboardButton("120m", callback_data="admin:auto:vf:set:interval:120"),
                    InlineKeyboardButton("240m", callback_data="admin:auto:vf:set:interval:240"),
                ],
                [
                    InlineKeyboardButton("qty 5", callback_data="admin:auto:vf:set:qty:5"),
                    InlineKeyboardButton("qty 10", callback_data="admin:auto:vf:set:qty:10"),
                    InlineKeyboardButton("qty 15", callback_data="admin:auto:vf:set:qty:15"),
                    InlineKeyboardButton("qty 20", callback_data="admin:auto:vf:set:qty:20"),
                ],
                [
                    InlineKeyboardButton("top 50", callback_data="admin:auto:vf:set:top_n:50"),
                    InlineKeyboardButton("top 100", callback_data="admin:auto:vf:set:top_n:100"),
                    InlineKeyboardButton("top 200", callback_data="admin:auto:vf:set:top_n:200"),
                ],
                [
                    InlineKeyboardButton("mode page", callback_data="admin:auto:vf:set:mode:page"),
                    InlineKeyboardButton("mode all", callback_data="admin:auto:vf:set:mode:all"),
                ],
                [
                    InlineKeyboardButton("head 10", callback_data="admin:auto:vf:set:head_k:10"),
                    InlineKeyboardButton("head 25", callback_data="admin:auto:vf:set:head_k:25"),
                    InlineKeyboardButton("head 50", callback_data="admin:auto:vf:set:head_k:50"),
                ],
                [
                    InlineKeyboardButton("⬅️ Voltar", callback_data="admin:auto"),
                ],
    ...
    def _coomer_creator_score(self, creator: dict) -> float:
        """Compute a best-effort popularity score for a creator.

        Coomer creator objects vary by service; we weight "followers/subscribers/favorites" much
        more than raw post_count, so the feed tends to pick bigger names.
        """
        ...
        def _f(key: str) -> float:
            ...
            try:
                ...
                return 0.0
        favorites = max(_f('favorite_count'), _f('favorites'), _f('favorited'))
        followers = max(_f('followers'), _f('subscribers'))
        posts = max(_f('posts'), _f('post_count'))
        # Big weights for popularity signals; tiny weight for sheer volume.
        score = favorites * 100.0 + followers * 10.0 + posts * 1.0
        # Small boost for being in the dataset at all (avoid zeros)
        if score <= 0:
            # Some creator objects may not expose counts; fall back to a minimal score so
            # API order (often popularity-ranked) still matters.
            score = 1.0
        return score
    async def _coomer_get_top_creators(self, fetcher: MediaFetcher, top_n: int = 100) -> list[dict]:
        """Return a filtered, ranked creators list (best-effort), cached for a short time."""
        now = time.time()
        ...
        # Prefer onlyfans/fansly for this bot's "model" use-case
        allowed_services = {'onlyfans', 'fansly'}
        filtered = [c for c in creators if isinstance(c, dict) and str(c.get('service', '')).lower() in allowed_services]
        if not filtered:
            filtered = [c for c in creators if isinstance(c, dict)]

        # Rank by score. If scores are flat/missing, keep API order.
        try:
            scored = [(self._coomer_creator_score(c), c) for c in filtered]
            max_score = max((s for s, _ in scored), default=0.0)
            if max_score > 1.0:
                creators_sorted = [c for s, c in sorted(scored, key=lambda t: t[0], reverse=True)]
            else:
                creators_sorted = filtered
        except Exception:
            creators_sorted = filtered

        creators_sorted = creators_sorted[:top_n]
        try:
    ...
    async def vip_feed_job(self) -> None:
        """One cycle: (1) auto previews (if enabled) and (2) seed VIP with a Coomer slot.

        Slot engine (A):
        - Each cycle selects ONE creator from the top list (weighted), rotates away from the last creator.
        - Sends up to page_items items from that creator, paging if needed.
        - Persists per-creator progress so when the creator returns later it doesn't repeat.
        """
        async with self._vip_feed_lock:
            ...
            page_items = max(1, min(int(page_items), 50))
            top_n = int(cfg.get("top_n", 100) or 100)
            mode = str(cfg.get("mode", "page") or "page").strip().lower()
            if mode not in ("page", "all"):
                mode = "page"
            head_k = int(cfg.get("head_k", 25) or 25)
            head_k = max(5, min(head_k, 100))

            # 1) Auto previews (FREE/admin) step keeps running (if enabled)
            try:
            ...
            async with MediaFetcher() as fetcher:
                ...
                try:
                    last_key = self.preview_db.get_coomer_state("vip", vip_chat_id)
                except Exception:
                    last_key = None

                tries = 0
                while tries < 5 and sent < 1:
                    tries += 1
                    pool = head if head else top_creators[: min(50, len(top_creators))]
                    creator = _weighted_choice(pool, exclude_key=last_key)
                    if not creator:
                    ...
                    creator_key = _creator_key(creator)
                    creator_label = str(creator.get("name") or creator.get("username") or creator.get("id") or "modelo")
                    service = str(creator.get("service") or "").strip().lower()
                    creator_id = str(creator.get("id") or creator.get("username") or "").strip()

                    try:
                        offset = int(self.preview_db.get_coomer_progress("vip", vip_chat_id, service, creator_id) or 0)
                    except Exception:
                        offset = 0

                    pages_tried = 0
                    max_pages = 1 if mode == "page" else 10
                    last_offset = int(offset or 0)

                    while sent < int(page_items) and pages_tried < max_pages:
                        pages_tried += 1
                        try:
                            media_items: list[MediaItem] = await fetcher.fetch_posts_paged(creator, offset=int(last_offset), smart_sort=True)
                        except Exception as e:
                            logger.warning("VIP feed: fetch_posts_paged failed (%s): %s", creator_label, e)
                            media_items = []
                        ...
                        for item in media_items:
                            ...
                            # Coomer fetch returns URL metadata only; download first.
                            try:
                                try:
                                    ok_dl = await fetcher.download_media(item)
                                except TooLargeMedia as tl:
                                    try:
                                        logger.info(
                                            "VIP feed: skipping oversized media (%.2f MB > %.2f MB) url=%s",
                                            (getattr(tl, "size_bytes", 0) or 0) / (1024 * 1024),
                                            (getattr(tl, "max_bytes", 0) or 0) / (1024 * 1024),
                                            getattr(tl, "url", None),
                                        )
                                ...
                                logger.warning("VIP feed: download failed (%s): %s", creator_label, e)
                                continue

                            caption = f"🔥 VIP Drop: {creator_label}"
                            try:
                                ok_up = await self.uploader.upload_and_cleanup(
                                    item,
                                    channel_id=int(vip_chat_id),
                                    caption=caption,
                                    model_name=creator_label,
                                )
                            except Exception as e:
                                logger.warning("VIP feed: upload failed (%s): %s", creator_label, e)
                                ok_up = False
                            ...
                            if ok_up:
                                ...
                                try:
                                    if gkey:
                                        self.preview_db.global_mark_sent(gkey, source="vip_feed")
                                except Exception:
                                    pass
                    ...
                    try:
                        if service and creator_id:
                            self.preview_db.set_coomer_progress("vip", vip_chat_id, service, creator_id, int(last_offset))
                    except Exception:
                        pass

                    try:
                        self.preview_db.set_coomer_state("vip", vip_chat_id, creator_key)
                    except Exception:
                        pass

                    last_key = creator_key

            logger.info(
                "VIP feed: cycle done interval=%sm top_n=%s page_items=%s mode=%s head_k=%s creator=%s sent=%s",
                interval_minutes, top_n, page_items, mode, head_k, creator_label, sent
            )
    ...
    # -----------------
    # Supervisor (keeps background loops alive)
    # -----------------
    def _supervisor_running(self) -> bool:
        try:
            return bool(self._supervisor_task and not self._supervisor_task.done())
        except Exception:
            return False

    def _start_supervisor_task(self) -> None:
        if not self.app:
            return
        ...
        try:
            ...
            pass
        self._supervisor_task = asyncio.create_task(self._supervisor_loop(), name="supervisor_loop")

    def _stop_supervisor_task(self) -> None:
        try:
    ...
    async def _supervisor_loop(self) -> None:
        """Periodically checks configs and restarts background tasks if needed.

        This prevents the "runs for 1h then stops" symptom when something transient
        (network/flood) causes a task to exit or when a config reload temporarily
        returns missing IDs.
        """
        ...
        while True:
            try:
                ...
                try:
                    cfg = self.preview_db.get_auto_config() or {}
                    enabled = int(cfg.get("enabled", 0) or 0) == 1
                    if enabled and not self._auto_task_running():
                        logger.warning("Supervisor: auto previews enabled but task not running -> starting")
                        self._start_auto_previews_task()
                    if (not enabled) and self._auto_task_running():
                        logger.info("Supervisor: auto previews disabled -> stopping")
                        self._stop_auto_previews_task()
                except Exception:
                    pass
                ...
                # VIP feed
                try:
                    vcfg = self.preview_db.get_vip_feed_config() or {}
                    venabled = int(vcfg.get("enabled", 0) or 0) == 1
                    if venabled and not self._vip_feed_task_running():
                        logger.warning("Supervisor: vip feed enabled but task not running -> starting")
                        self._start_vip_feed_task()
                    if (not venabled) and self._vip_feed_task_running():
                        logger.info("Supervisor: vip feed disabled -> stopping")
                        self._stop_vip_feed_task()
                except Exception:
                    pass
    ...
    async def cmd_vipfeed_on(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        Usage (backwards compatible):
          /vipfeed_on <interval_minutes> <page_items> [top_n]

        Extended:
          /vipfeed_on <interval_minutes> <page_items|all> [top_n] [page|all] [head_k]

        - page_items: 1..50 (Telegram-safe). Use 'all' to set page_items=50 and mode=all.
        - mode 'page': fetch a single page (offset) per cycle.
        - mode 'all': may traverse multiple pages of the selected creator per cycle until 'page_items' is reached.
        - head_k: how deep inside top_n we sample from to stay near the very top.
        """
        if not await self._admin_only(update, context):
            return
        args = list(context.args or [])
        if len(args) < 2:
            await self._safe_reply(update, context, "Uso: /vipfeed_on <interval_min> <itens|all> [top_n] [page|all] [head_k]", parse_mode=None)
            return

        interval_minutes = int(args[0])
        items_raw = str(args[1]).strip().lower()

        top_n = int(args[2]) if len(args) >= 3 else 100
        mode = str(args[3]).strip().lower() if len(args) >= 4 else None
        head_k = int(args[4]) if len(args) >= 5 else 25
        ...
        if items_raw == 'all':
            ...
            page_items = int(items_raw)

        if mode not in ('page', 'all'):
            mode = 'page'

        ok = self.preview_db.set_vip_feed_config(1, interval_minutes, page_items, top_n=top_n, mode=mode, head_k=head_k)
        if ok:
            self._start_vip_feed_task()
            cfg = self.preview_db.get_vip_feed_config() or {}
            await self._safe_reply(
                update,
                context,
                f"✅ VIP feed ON. interval={cfg.get('interval_minutes')}m qty={cfg.get('page_items')} mode={cfg.get('mode')} top_n={cfg.get('top_n')} head_k={cfg.get('head_k')}",
                parse_mode=None,
            )
        else:
            await self._safe_reply(update, context, "❌ Falha ao salvar config do VIP feed.", parse_mode=None)
    ...
    async def post_init(self, application: Application):
        ...
        # Supervisor (keeps loops alive)
        try:
            self._start_supervisor_task()
        except Exception as e:
            logger.warning("Supervisor init failed (ignored): %s", e)
