...
class VIPBot:
    ...
    async def _send_random_previews_from_coomer(self, bot, model: str, qty: int, dest_chat_ids: list[int]) -> Tuple[int, int]:
        ...
        v6+ slot engine:
        - Each destination keeps a persistent cursor (creator + offset).
        - We send a batch from ONE creator per destination per cycle.
        ...
        async with MediaFetcher() as fetcher:
            # If a specific model was requested, resolve it once.
            fixed_creator: dict | None = None
            if model_l not in ("random", "all", "*", "popular", "top"):
                try:
                    fixed_creator = await fetcher.find_creator(model_q)
                except Exception:
                    fixed_creator = None

            # Per-destination batch
            for dest in dest_chat_ids:
                ...
                cursor = None
                try:
                    cursor = self.preview_db.get_coomer_cursor("free", dest_id)
                except Exception:
                    cursor = None
                ...
                # If fixed creator requested, ignore stored cursor creator.
                if fixed_creator:
                    creator = fixed_creator
                    offset = int(cursor.get("offset", 0) if cursor and cursor.get("creator_id") == str(fixed_creator.get("id")) else 0)
                else:
                    # Continue existing slot if possible
                    if cursor and cursor.get("service") and cursor.get("creator_id"):
                        creator = {"service": cursor.get("service"), "id": cursor.get("creator_id"), "name": cursor.get("creator_name") or ""}
                        offset = int(cursor.get("offset", 0) or 0)

                # If no slot, pick a new creator from top N
                if not creator or not creator.get("service") or not creator.get("id"):
                    try:
                    ...
                    top_n = all_creators[:100] if len(all_creators) > 100 else all_creators
                    # Weighted-ish choice favouring higher ranks
                    picked = None
                    for _ in range(6):
                        if not top_n:
                            break
                        # pick from a tighter head for quality
                        head = top_n[: min(40, len(top_n))]
                        try:
                            picked = random.choice(head)
                        except Exception:
                            picked = head[0]
                        if picked:
                            break
                    ...
                    try:
                        self.preview_db.set_coomer_cursor(
                            "free",
                            dest_id,
                            service=str(creator.get("service") or ""),
                ...
                # Try a few pages to complete qty (skip already-sent or too-large)
                pages_tried = 0
                last_offset = int(offset or 0)
                exhausted = False
                while sent_for_dest < int(qty) and pages_tried < 4:
                    pages_tried += 1
                    media_items: list[MediaItem] = []
                    try:
                        media_items = await fetcher.fetch_posts_paged(creator, offset=int(last_offset), smart_sort=True)
                    except Exception as e:
                    ...
                    for media in media_items:
                        if sent_for_dest >= int(qty):
                            break
                        if not getattr(media, "url", None):
                            continue

                        # Global de-dup (automatic modules)
                        gkey = self._coomer_media_key_str(creator, media)
                        try:
                        ...
                        # Per-dest de-dup (legacy)
                        try:
                            key_int = self._coomer_item_key(creator, media)
                            if self.preview_db.has_sent(dest_chat_id=dest_id, model="coomer", vip_chat_id=0, vip_message_id=int(key_int)):
                                continue
                        except Exception:
                            pass
                        ...
                        try:
                            ...
                            try:
                                logger.info(
                                    "Coomer autopreviews: skipping oversized media (%.2f MB) url=%s",
                        ...
                        try:
                            ...
                            if msg_id:
                                sent_for_dest += 1
                                sent_count += 1
                                # mark per-dest + global
                                try:
                                    self.preview_db.mark_sent(dest_chat_id=dest_id, model="coomer", vip_chat_id=0, vip_message_id=int(self._coomer_item_key(creator, media)), file_unique_id=str(getattr(media, "filename", "") or ""))
                                except Exception:
                                    pass
                                try:
                    ...
                    # Move cursor forward for next cycle
                    last_offset = int(last_offset) + 50

                # Persist cursor: if exhausted, clear slot so next cycle picks a new creator
                try:
                    if exhausted:
                        self.preview_db.clear_coomer_cursor("free", dest_id)
                    else:
                        self.preview_db.set_coomer_cursor(
                            "free",
                            dest_id,
    ...
    async def vip_feed_job(self) -> None:
        ...
        async with self._vip_feed_lock:
            ...
            if mode not in ("page", "all"): mode = "page"
            ...
            # Slot engine for VIP seeding: continue a creator until we can't find new items.
            offsets = [0, 50, 100, 150, 200, 250, 300, 350, 400, 450]
            ...
            try:
                async with MediaFetcher() as fetcher:
                    ...
                    cursor = None
                    try:
                        cursor = self.preview_db.get_coomer_cursor("vip", int(vip_chat_id))
                    except Exception:
                        cursor = None
                    ...
                    if cursor and cursor.get("service") and cursor.get("creator_id"):
                        creator = {
                            ...
                            "id": cursor.get("creator_id"),
                            "name": cursor.get("creator_name") or "",
                        }
                        offset = int(cursor.get("offset", 0) or 0)
                    else:
                        ...
                        # Favor the head for quality: sample only within the top head_k
                        head = top_creators[: min(head_k, len(top_creators))]
                        # Weighted random: bias to higher popularity signals
                        try:
                            weights = [self._coomer_creator_score(c) for c in head]
                            creator = random.choices(head, weights=weights, k=1)[0]
                        except Exception:
                            try:
                                creator = random.choice(head)
                            except Exception:
                                creator = head[0]
                        offset = 0  # start from newest page for reliability; cursor advances each cycle

                    if not creator:
                        return

                    creator_name = str(creator.get("name") or creator.get("username") or creator.get("id") or "unknown")
                    ...
                    while sent < int(page_items) and pages_tried < max_pages:
                        ...
                        if not media_items:
                            # If we were resuming a cursor and there are no items, consider this creator exhausted.
                            if cursor and cursor.get("service") and cursor.get("creator_id"):
                                exhausted = True
                                break

                            # When starting fresh, some creators/offsets return empty even though others work.
                            # Try a few different top creators (always from offset 0) before giving up this cycle.
                            retry_creators += 1
                            if retry_creators <= 5 and 'head' in locals() and head:
                                try:
                                    creator = random.choice(head)
                                except Exception:
                                    creator = head[0]
                        ...
                        for item in media_items:
                            ...
                            gkey = self._coomer_media_key_str(creator, item)
                            try:
                                if self.preview_db.global_has_sent(gkey):
                                    continue
                            except Exception:
                            ...
                            try:
                                ...
                                logger.warning("VIP feed: upload failed (%s): %s", creator_name, e)
                                ok = False
                            if ok:
                                sent += 1
                                try:
                    ...
                    try:
                        if exhausted:
                            self.preview_db.clear_coomer_cursor("vip", int(vip_chat_id))
                        else:
                            self.preview_db.set_coomer_cursor(
                                "vip",
                                int(vip_chat_id),
