...
class VIPBot:
    ...
    async def cmd_autopreviews_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not dest:
            if send_to_free and send_to_private:
                ...
                dest = "admin"
        free_mode = cfg.get("free_target_mode") or cfg.get("free_mode") or "all"
        free_chat_id = cfg.get("free_target_chat_id") or cfg.get("free_chat_id")
    ...
    async def auto_previews_job(self, context: ContextTypes.DEFAULT_TYPE = None, cfg_override: dict | None = None):
        ...
        async with self._auto_previews_lock:
            ...
            dest = cfg.get("dest")
            # DB-compatible keys: send_to_free/send_to_private + free_mode/free_chat_id
            send_to_free = int(cfg.get("send_to_free", 0) or 0)
            send_to_private = int(cfg.get("send_to_private", 0) or 0)
            if not dest:
                if send_to_free and send_to_private:
            ...
            targets: list[int] = []
            if dest in ("admin", "private"):
                if admin_chat_id:
                    targets = [int(admin_chat_id)]
            else:
                if free_mode == "single" and free_chat_id:
                    targets = [int(free_chat_id)]
                else:
                    targets = [int(x) for x in (config.FREE_CHANNELS or [])]
