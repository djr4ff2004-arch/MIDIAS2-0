...
class PreviewIndex:
        ...
        try:
                ...
                try:
                    with self._pg_conn() as conn:
                        with conn.cursor() as cur:
                            ...
                            FROM auto_previews_config WHERE id=1;
                            """
                        )
                        row = cur.fetchone()
                        if not row:
                        ...
                        return {
                            ...
                            "qty": int(row[2]),
                            "model": str(row[3]),
                            "send_to_free": int(row[4]),
                            "send_to_private": int(row[5]),
                            "free_mode": str(row[6]) if row[6] is not None else "all",
                            "free_chat_id": None if row[7] is None else int(row[7]),
                            "admin_chat_id": None if row[8] is None else int(row[8]),
                        }
                except Exception as e:
                    logger.warning(
                        f"PreviewIndex Postgres get_auto_config failed; falling back to SQLite: {e}"
                    )
                    self.use_postgres = False
            ...
            with self._sqlite_conn() as conn:
                ...
                return {
                    ...
                    "qty": int(row["qty"]),
                    "model": str(row["model"]),
                    "send_to_free": int(row["send_to_free"]),
                    "send_to_private": int(row["send_to_private"]),
                    "free_mode": str(row["free_mode"]) if row["free_mode"] is not None else "all",
                    "free_chat_id": None if row["free_chat_id"] is None else int(row["free_chat_id"]),
                    "admin_chat_id": None if row["admin_chat_id"] is None else int(row["admin_chat_id"]),
                }
        except Exception as e:
    ...
    def set_vip_feed_config(
        ...
        This module periodically seeds the VIP channel with a random page from a random
        creator among the top-N creators returned by the Coomer creators endpoint.
        """
                        ...
                        with conn.cursor() as cur:
                            cur.execute(
                                ...
                                INSERT INTO vip_feed_config
                                    (id, enabled, interval_minutes, page_items, top_n, updated_at)
                                ...
                                DO UPDATE SET
                                    ...
                                    updated_at=NOW();
                                """,
                                (int(enabled), int(interval_minutes), int(page_items), int(top_n)),
                            )
                        conn.commit()
                    return True
                except Exception as e:
                ...
                try:
                    with self._pg_conn() as conn:
                        with conn.cursor() as cur:
                            cur.execute(
                                """
                                SELECT enabled, interval_minutes, page_items, top_n
